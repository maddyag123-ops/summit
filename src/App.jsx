  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session) => {
      if (session?.user) {
        ...
        setAuthReady(true);
        console.log('[Auth] authReady set, adminFlag:', adminFlag, 'raw metadata:', session?.user?.user_metadata);
        setUserId(uid);
        setLoading(true);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-slate-950/95 backdrop-blur-xl border-t border-slate-800/50 z-50">
        <div className="max-w-2xl mx-auto flex">
          ...
          {console.log('[Nav] authReady:', authReady, 'isAdmin:', isAdmin)}
...
function DashboardView({ dailyData, ewmaData, fingerEWMAData, weekOnOffSplit, datesSorted, assessData, climbData }) {
  ...
  const forceComparisonData = useMemo(() => {
    return rangeDates
      .map(d => {
        const dc = climbData[d];
        if (!dc?.baselineL || !dc?.postSessionPeakL) return null;
        const baseline = (Number(dc.baselineL) + Number(dc.baselineR || dc.baselineL)) / 2;
        const postSession = (Number(dc.postSessionPeakL) + Number(dc.postSessionPeakR || dc.postSessionPeakL)) / 2;
        return { date: d.slice(5), baseline: baseline.toFixed(1), postSession: postSession.toFixed(1) };
      })
      .filter(Boolean)
      .slice(-14);
  }, [climbData, rangeDates]);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-3">
        {chart === "ewma" && <>{ewmaCD.length > 1 ? <><div className="text-[10px] text-slate-500 uppercase tracking-wider mb-3">EWMA Acute vs Chronic</div><ResponsiveContainer width="100%" height={240}><LineChart data={ewmaCD} margin={{ top: 5, right: 5, bottom: 5, left: -15 }}><CartesianGrid strokeDasharray="3 3" stroke="#1e293b" /><XAxis dataKey="date" tick={{ fill: "#64748b", fontSize: 9 }} interval="preserveStartEnd" /><YAxis tick={{ fill: "#64748b", fontSize: 9 }} /><Tooltip contentStyle={ttStyle} /><Legend wrapperStyle={{ fontSize: 11, cursor: 'pointer' }} onClick={(e) => toggleLine(e.dataKey)} formatter={(value, entry) => (<span style={{ color: hiddenLines[entry.dataKey] ? '#475569' : entry.color, textDecoration: hiddenLines[entry.dataKey] ? 'line-through' : 'none' }}>{value}</span>)} /><Line type="monotone" dataKey="acute" stroke="#38bdf8" strokeWidth={2} dot={false} hide={hiddenLines['acute']} name="Acute" /><Line type="monotone" dataKey="chronic" stroke="#94a3b8" strokeWidth={2} strokeDasharray="5 5" dot={false} hide={hiddenLines['chronic']} name="Chronic" /><Line type="monotone" dataKey="fingerAcute" stroke="#FBBF24" strokeWidth={1.5} strokeDasharray="4 4" dot={false} hide={hiddenLines['fingerAcute']} name="Finger Acute" /><Line type="monotone" dataKey="fingerChronic" stroke="#F59E0B" strokeWidth={1} strokeDasharray="4 4" dot={false} hide={hiddenLines['fingerChronic']} name="Finger Chronic" /></LineChart></ResponsiveContainer></> : <div className="text-center text-slate-600 py-12 text-sm">Log 2+ days</div>}</>}
        ...
        {chart === "force" && <>{forceCD.length > 1 ? <><div className="text-[10px] text-slate-500 uppercase tracking-wider mb-3">Daily Force — Left / Right / Average</div><ResponsiveContainer width="100%" height={280}><LineChart data={forceCD} margin={{ top: 5, right: 5, bottom: 5, left: -15 }}><CartesianGrid strokeDasharray="3 3" stroke="#1e293b" /><XAxis dataKey="date" tick={{ fill: "#64748b", fontSize: 9 }} interval="preserveStartEnd" /><YAxis tick={{ fill: "#64748b", fontSize: 9 }} /><Tooltip contentStyle={ttStyle} /><Legend wrapperStyle={{ fontSize: 10 }} />{forceCD.some(d => d.tindeqL) && <Line type="monotone" dataKey="tindeqL" stroke="#38bdf8" strokeWidth={1.5} dot={false} name="Tindeq L" />}{forceCD.some(d => d.tindeqR) && <Line type="monotone" dataKey="tindeqR" stroke="#0ea5e9" strokeWidth={1.5} dot={false} name="Tindeq R" />}{forceCD.some(d => d.tindeqAvg) && <Line type="monotone" dataKey="tindeqAvg" stroke="#7dd3fc" strokeWidth={2} strokeDasharray="6 3" dot={false} name="Tindeq Avg" />}{forceCD.some(d => d.gripL) && <Line type="monotone" dataKey="gripL" stroke="#a78bfa" strokeWidth={1.5} dot={false} name="Grip L" />}{forceCD.some(d => d.gripR) && <Line type="monotone" dataKey="gripR" stroke="#8b5cf6" strokeWidth={1.5} dot={false} name="Grip R" />}{forceCD.some(d => d.gripAvg) && <Line type="monotone" dataKey="gripAvg" stroke="#c4b5fd" strokeWidth={2} strokeDasharray="6 3" dot={false} name="Grip Avg" />}</LineChart></ResponsiveContainer></> : <div className="text-center text-slate-600 py-12 text-sm">Log force</div>}
          {forceComparisonData.length > 0 && <div className="mt-4 pt-4 border-t border-slate-800/50">
            <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-0.5">Session Fatigue: Baseline vs Post-Session</div>
            <div className="text-[10px] text-slate-600 mb-3">Average of L/R readings per session.</div>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={forceComparisonData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#64748b' }} />
                <YAxis tick={{ fontSize: 10, fill: '#64748b' }} />
                <Tooltip contentStyle={{ background: '#1e293b', border: '1px solid #334155', fontSize: 11 }} />
                <Legend wrapperStyle={{ fontSize: 11 }} />
                <Bar dataKey="baseline" fill="#38bdf8" name="Morning Baseline" radius={[3, 3, 0, 0]} />
                <Bar dataKey="postSession" fill="#f59e0b" name="Post-Session" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>}
        </>}
